...
import { FloatingInfoButton } from "./info-button"
...
interface EditorProps {
  ...
  showExportButton?: boolean // Show the export/code button in bottom right - default: false
  showTemplates?: boolean // Show template switcher button in bottom left - default: false
  onPreviewClick?: () => void // Callback when preview button is clicked (opens in new tab)
  pageBackgroundColor?: string // Background color of the page content area
  onPageBackgroundColorChange?: (color: string) => void // Callback when page background color changes
  infoButtonConfig?: import("./info-button").InfoButtonConfig // Config for the floating info button
  onInfoButtonConfigChange?: (config: import("./info-button").InfoButtonConfig) => void // Callback when info button config changes
}
...
export function Editor({
  ...
  useEffect(() => {
    ...
    const attemptFocus = (retries = 0) => {
      ...
      // Silently fail if element not found - common for images/media that don't need focus
    }
    ...
    <div className="flex flex-1 flex-col transition-colors duration-300">
      ...
      <div className="mx-auto flex w-full flex-1 flex-col">
        ...
        <EditorToolbar
          ...
          onInsertComponentClick={() => {}} // Kept for interface compatibility
          ...
          onInfoButtonConfigChange={onInfoButtonConfigChange}
        />
                  ...
                  className={cn(
                    ...
                    notionBased && coverImage
                      ...
                      : notionBased
                        ? "pt-[30px]"
                        : "pt-4",
                    // Document-style: dark text on white background
                    "text-gray-900 [&_h1]:text-gray-900 [&_h2]:text-gray-800 [&_h3]:text-gray-800 [&_p]:text-gray-700 [&_li]:text-gray-700 [&_blockquote]:text-gray-600 [&_pre]:text-gray-800 [&_code]:text-gray-800"
                  )}
                >
      ...
      {/* Floating Export Button - Only show when explicitly enabled */}
      ...
      {/* Floating Info Button - Preview only (admin sees it in editor preview) */}
      {infoButtonConfig && infoButtonConfig.enabled && (
        <FloatingInfoButton config={infoButtonConfig} />
      )}

      {/* Template Switcher Button - Bottom Left - Only show when explicitly enabled */}
